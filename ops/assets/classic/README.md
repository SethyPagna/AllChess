...
The Blender delivery image and desktop/mobile browser renders were inspected. Automated checks parse the actual GLBs, confirm all semantic roots and playable dimensions, reject external asset dependencies, and keep camera framing inside the viewport. Browser audits check lazy loading, all nine supported game views, raycast moves and board rotation, castling, promotion, Crazyhouse drops, saved preferences, and 320/390-pixel layouts. The current tabletop audit additionally verifies orbit/zoom/reset, Cambodia and classic moves, unboxed 2D coordinates, and 320/390-pixel layouts. Failed downloads and lost WebGL contexts offer a direct 2D fallback without resetting the game.
