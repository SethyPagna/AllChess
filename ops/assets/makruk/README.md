...
Setup and movement references: [GNU XBoard Makruk](https://www.gnu.org/software/xboard/whats_new/rules/Makruk.html) and [PyChess Makruk](https://www.pychess.org/variants/makruk). White's Khun now starts on d1, Black's on e8, each to the player's left of Met. Existing saved boards are not rewritten. Pawn promotion is on the sixth rank. A subsequent pass adds versioned optional board honor and fixed piece honor, while preserving legacy saves. See the [counting profile](../../docs/roadmap/makruk-counting-profile.md) for implementation, verification and remaining ranked/tournament gates.
