...
- Both seats are reserved by credential digest before the room link is returned. A third browser may watch but cannot occupy either seat. Clocks wait until both assigned players join within a 60-second arrival window.
...
A compact arrival panel shows the remaining server time and a Cancel match button. If both players join before the deadline, the panel disappears and play starts. At the deadline, admission expires before a late join can start play. Either assigned player can cancel while the room is still waiting. The game ID and seat credential are required; spectators cannot cancel. A cancellation racing with the second arrival either closes admission or receives a conflict because the game already started. It never resigns an active game automatically.
