...
- Future long-form research can still move into `ops/docs/` and link back from the root files.
...
**Decision:** Move `plan.md`, `progress.md`, `decisions.md`, and `change-log.md` into `ops/docs/roadmap/`, and keep README as the root pointer.
...
- Future planning artifacts should join `ops/docs/roadmap/` unless they are runtime-facing documentation.
...
- `ops/tests/maintenance/project-organization.test.ts` and `ops/tests/maintenance/markdown-docs.test.ts` guard the structure against drift.
...
- `ops/tests/maintenance/project-organization.test.ts` allows only `next.config.mjs` as first-party JavaScript.
...
- `ops/tests/maintenance/markdown-docs.test.ts` guards the docs folder shape.
