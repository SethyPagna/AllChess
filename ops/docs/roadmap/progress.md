...
**Current Status:** The active product goal continues on `codex/compact-game-studio`. Latest delivery: [mobile tabletop framing and controls](mobile-tabletop.md), with compact physical Shogi hand trays, fitted angled cameras and gesture protection. This follows [Jungle tabletop and rules profile](jungle-tabletop.md), completing native 3D coverage for all 21 playable games, [Shatranj and Chaturanga tabletops](historical-tabletops.md), [Kōnane papamū and NPS profile](konane-tabletop.md) and [physical draughts boards](draughts-tabletops.md). The original phase table below is a historical planning baseline; individual delivery notes record current scope and validation.
...
- Deployment and runtime configuration live in `ops/infra/cloudflare/wrangler.jsonc`, `ops/infra/docker/*`, `config/test/*`, `config/lint/*`, `config/typescript/*`, `open-next.config.ts`, `vercel.json`, Cloudflare docs, and package scripts.
