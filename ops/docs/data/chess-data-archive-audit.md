...
The current `data/local/chess-data` archives and extracted folders are still referenced by `ops/scripts/training/train-bot-knowledge.ts` and by the generated runtime manifest at `src/data/bot-knowledge.generated.json`. Keep this folder ignored and local unless a compact generated artifact is intentionally committed.
...
2. It is not needed by `ops/scripts/training/train-bot-knowledge.ts`.
