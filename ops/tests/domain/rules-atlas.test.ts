...
describe("rules atlas", () => {
  ...
  test("captures key native rule differences in plain language", () => {
    ...
    expect(getVariantRuleSummary("makruk").completion.verifiedEdgeCases).toEqual(expect.arrayContaining([expect.stringContaining("Versioned honor counting")]));
    expect(getVariantRuleSummary("makruk").completion.remainingGates).toEqual(expect.arrayContaining([expect.stringContaining("Ranked matchmaking")]));
    ...
    expect(getVariantRuleSummary("international-draughts").numberedBasics[2]).toContain("flying kings");
    ...
    expect(getVariantRuleSummary("konane").numberedBasics[1]).toContain("remove");
    expect(getVariantRuleSummary("konane").completion.verifiedEdgeCases).toEqual(expect.arrayContaining([expect.stringContaining("Opening removals")]));
    expect(getVariantRuleSummary("konane").completion.status).toBe("verified-playable");
    expect(getVariantRuleSummary("konane").completion.remainingGates).toEqual([]);
