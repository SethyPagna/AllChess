...
export function encodeLocalMatch(snapshot: LocalMatchSnapshot): string {
  ...
  const nodes: unknown[] = [], keys = new Map<string, number>();
  function intern(item: unknown, depth = 0): number {
    ...
    if (Array.isArray(item)) node = { a: item.map(child => intern(child, depth + 1)) };
    else if (item && typeof item === "object") node = { o: Object.entries(item).filter(([, child]) => child !== undefined).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([key, child]) => [intern(key, depth + 1), intern(child, depth + 1)]) };
    ...
    const index = nodes.push(node) - 1; keys.set(key, index); return index;
  }
...
export function decodeLocalMatch(record: LocalMatchRecord): LocalMatchSnapshot | null {
  try {
    ...
    const values: unknown[] = [], depths: number[] = []; let edges = 0;
    for (const node of packed.nodes) {
      ...
      const dereference = (index: number) => {
        ...
        depth = Math.max(depth, depths[index] + 1); return values[index];
      };
      let value: unknown;
      if (node === null || typeof node === "string" || typeof node === "boolean" || (typeof node === "number" && Number.isFinite(node))) value = node;
      ...
      values.push(value); depths.push(depth);
    }
