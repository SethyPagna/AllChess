...
export function BoardGrid({ cols, files, legalTargets, legalTargetMode = "move", locale = "en", onChoose, onDragMove, onDropHandPiece, orientedRows, pieceSkin = "default", rows, selected, suggestedMove, lastMove, variantKey }: BoardGridProps) {
  ...
  const plainBoard = variantKey === "ouk-chaktrang" || intersectionBoard || variantKey === "shogi" || variantKey === "mini-shogi" || variantKey === "makruk" || variantKey === "turkish-draughts";
  ...
  return (
    <div
      ...
      data-board-geometry={intersectionBoard ? "intersections" : plainBoard ? "plain-grid" : "checkered"}
      ...
      onPointerMoveCapture={(event) => {
        ...
        setDragGhost((current) => (current ? { ...current, left: event.clientX, top: event.clientY } : current));
      }}
